...
distances(x; dist=Euclidean()) = pairwise(dist, x, dims=2)
distances(x, y; dist=Euclidean()) = pairwise(dist, x, y, dims=2)
...
function H(rs, x::AbstractMatrix, step::Real=rs.step.hi; dist=Euclidean())
    xs = 0.:step:1.
    H(rs, x, xs; dist)
end
